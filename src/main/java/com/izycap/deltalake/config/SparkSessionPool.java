package com.izycap.deltalake.config;
...
public class SparkSessionPool {
    ...
    public SparkSessionPool(
            @Value("${spark.cluster.URL}") String sparkClusterUrl,
            @Value("${s3a.URL}") String s3aUrl,
            @Value("${s3a.accessKey}") String accessKey,
            @Value("${s3a.secretKey}") String secretKey,
            @Value("${spark.maxSparkSessionInstances:10}") Integer maxSparkSessionInstances
    )
    {
        SparkSession sparkSession = SparkSession.builder()
                ...
                .config("spark.master", sparkClusterUrl)
                .getOrCreate();
